...
@section('content')
    ...
    <div class="d-flex bd-highlight mb-3">
        <div class="align-self-center mt-3">
            <h5 class=" pageMainTitle mb-3">Department Details</h5>
        </div>
        ...
        <div class="p-2 bd-highlight">
            <button type="button" class="btn btn-primary lead" onclick="location.href='{{ route('company.create') }}'">
                <i class="fa fa-plus" aria-hidden="true" style="font-size:smaller;"></i> Add Department
            </button>
        </div>
    </div>
    ...
    <table class="table table-table table-light">
        ...
        <tbody>
        @forelse ($department as $key=> $dept)
            <tr>
                <th scope="row">
                ...
                <td><span style="color:#4046DD!important;">{{ $dept['staffNumber'] }}</span></td>
                <td>
                    <div class="d-flex bd-highlight mb-3">
                        <div class="align-self-center p-3 bd-highlight">
                            <h6>{{ $dept['company']->name}}</h6>
                            <span class="profile-customer"> {{  $dept['company']->email }} | {{ $dept['company']->phone_number }}
                                </span>
                        </div>
                    </div>
                </td>
...
{{--                        <a href="{{ route('profile.userPage', $dept->id) }}" class="actionBtnHolder editColor"> <i--}}
{{--                                class="fa fa-user customer"></i></a>--}}
{{--                    @endif--}}
                    <a href="" class="actionBtnHolder editColor" data-toggle="modal" data-target="#commonModal"
                       onclick="loadModal('{{ route('company.phdateModal', $dept->id) }}')">
                        <i class="fa fa-edit customer"></i>
                    </a>
                    {{-- <a href="{{ route('company.delete', $dept->id) }}">
                    ...
                    <a href="" class="actionBtnHolder" data-toggle="modal" data-target="#commonModal"
                       onclick="loadModal('{{ route('company.viewOwnCompany', $dept->id) }}')"
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <a href="{{ route('company.delete', $dept->id) }}" class="btn btn-danger">Delete</a>
                        </div>
                    </div>
                </div>
            </div>
