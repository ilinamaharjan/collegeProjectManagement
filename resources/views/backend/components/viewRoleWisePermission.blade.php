    ...
    <div class="row p-2">
        ...
        <div class="col mt-2">
            <input permission-name="{{$action['name']}}"
                {{$role->hasPermissionTo($action['id'])?'checked disabled':''}} value="{{$action['id']}}"
...
<div class="mt-2" id="permission_module_update_{{$rand}}">
    <div class="row p-3">
        <div class="col-md-12 p-1" style="background-color:#ddd;" p-2 text-light text-center" style="background-color:gray;border-radius:5px">
            <label><b class="text-dark ">{{$permissions['moduleName']}}</b></label> &nbsp;&nbsp;&nbsp;
            <input type="checkbox" onchange="handlePermissionCheck(`{{$rand}}`)"  id="parentPermission{{ $rand }}">
        </div>
    </div>
